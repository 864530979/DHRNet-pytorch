...
class OverlapPatchEmbed(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=7, stride=4, in_chans=3, embed_dim=768):
        ...
        if stride >0:
            ...
            self.proj = nn.Conv2d(in_chans, embed_dim, kernel_size=patch_size, dilation=-stride,
...
class VAN(nn.Module):
    def __init__(self, img_size=224, in_chans=3, num_classes=1000, embed_dims=[64, 128, 256, 512],
                mlp_ratios=[4, 4, 4, 4], drop_rate=0., drop_path_rate=0., norm_layer=nn.LayerNorm,
                 depths=[3, 4, 6, 3], num_stages=4, flag=False,out_layers=4):
        super().__init__()
...
def van_b0(pretrained=False, **kwargs):
    ...
    if pretrained:
        ...
        checkpoint = torch.load('./van_tiny_754.pth.tar')['state_dict']
...
def van_b1(pretrained=False, **kwargs):
    ...
    if pretrained:
        ...
        if pretrained:
            ...
            checkpoint = torch.load('./van_small_811.pth.tar')['state_dict']
            del checkpoint["head.weight"]
            del checkpoint["head.bias"]
            result = model.load_state_dict(checkpoint)
            print(result)
...
def van_b2(pretrained=False, **kwargs):
    ...
    if pretrained:
        # model = load_model_weights(model, "van_b2", kwargs)
        checkpoint = torch.load('./van_base_828.pth.tar')['state_dict']
        del checkpoint["head.weight"]
        del checkpoint["head.bias"]
        result = model.load_state_dict(checkpoint)
        print(result)
...
if __name__ == '__main__':
    net = van_b2(pretrained=True).cuda()
